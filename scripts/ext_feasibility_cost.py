...
EXT_COST_I18N_EN = {
    ...
    "subtotal": "Subtotal",
    "baseRisk": "Construction & equipment risk (20%)",
    "otherCont": "Contingency (20%)",
    "confidence": "Confidence contingency (30%)",
    "riskReg": "Risk register (factored costs)",
    ...
    "bullets": {
        ...
        "int": ["Wall finishes", "Ceilings", "Resin floors"],
        "fit": ["Sanitary / general fittings"],
        "bld": ["BWIC", "HVAC", "Electrical", "EMS", "Process controls"],
...
function extCostItemHTML(id,amt,lines){
...
inner+=`<div class="cost-line-items">${lines.map(([a,c])=>`<div class="cost-leaf"><span>${c}</span><span>${fm(a)}</span></div>`).join("")}</div>`;
}
const body=inner?`<div class="cost-children">${inner}</div>`:"";
return `<details class="cost-item"><summary><span class="cost-label">${L[id]}</span><span class="cost-amt">${fm(amt)}</span></summary>${body}</details>`;
}
function extCostHTML(){
const D=EXT_COST_DATA,L=extCostLabels();
let baseItems=D.baseItems.map(it=>extCostItemHTML(it.id,it.amount,it.lines||[])).join("");
const baseBlock=`<details class="cost-section" open><summary class="cost-section-hd"><span>${L.secBase}</span></summary>
...
const otherBlock=`<details class="cost-section" open><summary class="cost-section-hd"><span>${L.secOther}</span></summary>
...
const genBlock=`<details class="cost-section" open><summary class="cost-section-hd"><span>${L.secGen}</span></summary>
<div class="cost-section-body">
<div class="cost-leaf"><span>${L.confidence}</span><span>${fm(D.confidence)}</span></div>
<div class="cost-leaf"><span>${L.riskReg}</span><span>${fm(D.riskReg)}</span></div>
...
EXT_COST_CSS = """
.cost-section{border:1px solid #d0d8e0;border-radius:10px;margin-bottom:.45rem;background:#fafbfc}
.cost-section>summary.cost-section-hd{display:flex;justify-content:space-between;padding:.5rem .65rem;cursor:pointer;list-style:none;font-size:.84rem;font-weight:700;color:var(--navy);background:linear-gradient(180deg,#f0f4f8,#fff)}
.cost-section>summary::-webkit-details-marker{display:none}
.cost-section-body{padding:0 .45rem .45rem}
.cost-subtotal,.cost-risk-line{display:flex;justify-content:space-between;padding:.35rem .55rem;font-size:.78rem;border-top:1px dashed #dde3e8;margin-top:.2rem}
.cost-risk-line{color:var(--muted);font-style:italic}
.cost-section-total{display:flex;justify-content:space-between;padding:.45rem .55rem;margin-top:.25rem;border-radius:6px;background:var(--navy);color:#fff;font-size:.82rem;font-weight:700}
.cost-bullets{list-style:disc;margin:.25rem 0 .2rem 1.1rem;font-size:.68rem;color:var(--muted);line-height:1.35}
.cost-bullets li{margin-bottom:.1rem}
.cost-line-items{border-top:1px solid #f0f2f5;margin-top:.2rem;padding-top:.15rem}
