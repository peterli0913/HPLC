...
# --------------------------------------------------------------------------
# Re-baseline: the three earlier programmes assumed a June 2026 start; the
# portfolio now starts in September 2026, so every not-yet-started bar moves
# by three calendar months. Completed bars ("done") stay where they are.
# --------------------------------------------------------------------------
SHIFT_MONTHS = 3
TODAY = "2026-08-20"
...
# Tooltips carrying absolute dates need the same +3 months applied to the text.
EXT_TOOLTIP_PATCH = {
    "gR2": (
        "RIBA 2 Concept Design (160 days). Concept report & stage gate May 2027.",
        "RIBA 2 概念设计（160 天）；2027-05 概念报告与阶段评审。",
    ),
    "gPlan": (
        "Planning: submission Mar 2028, statutory period, consent target 21 Jun 2028.",
        "规划：2028-03 提交，法定审批期，目标许可 2028-06-21。",
    ),
    "gEnd": (
        "Validation, handover, project complete (07 Aug 2030).",
        "确认验证、移交；项目完成 2030-08-07。",
    ),
}
...
GANTT_EXT_SHIFTED = _patch_tooltips(shift_gantt(ext.GANTT_JS), EXT_TOOLTIP_PATCH)
...
<script>
const GANTT_EXT = ''' + json.dumps(GANTT_EXT_SHIFTED) + r''';
...
p2r1n:"B902 东侧扩建", p2r1s:"新建四层+夹层，反应/加氢/过滤干燥", p2r1o:"£78.1M", p2r1d:"2030-08 竣工",
...
p2rebase:"进度口径：前三条线原按 2026 年 6 月启动估算，现统一以 2026 年 9 月启动重新基线（整体顺延 3 个月），范围与费用不变；高活实验室概念进度本身即以 2026-09-01 资金批准为起点。",
...
extGSub:"总控时间轴（条块长度按日历比例）· 以 2026-09 启动重新基线，原 6 月口径整体 +3 个月",
...
hipoB1:"范围：既有 G-128 套间改造为实验室与书写区，GIFA 215 m²；隔离器按暂列金额计（4 台 × £150,000 + 1 台 × £100,000）。",
...
hipoB5:"待确认：概念阶段文件未定义 OEB 目标等级（隔离器按暂列金额计）；业主供货设备明细依据另一份设备清单，本资料未含。",
...
hipoS2:"隔离器与实验设备", hipoS2a:"隔离器暂列：4 台 × £150,000 + 1 台 × £100,000（合计 £700,000）",
hipoS2b:"通风柜：假设现有可继续使用，仅列 £20,000 维修保养暂列；是否新购为进度中待定项",
hipoS2c:"SF6 检漏测试暂列 £20,000；家具暂列含更衣柜、跨越凳、移动实验台、BIBO 桶",
...
hipoRiskNote:"风险登记册（2026-08-02 起草，12 项全部 Open，责任方 Employer）：净额 £397,875，区间 £146,633 – £649,117。最可能影响居前：AHU 状况不足需更换（最可能 £250,000）、最终用户需求变更（£200,000）、初始预算不足（£200,000）、设计发展变更（£95,000）；另有隔离器制造交期由约 3 个月延长至 5–8 个月的风险（按 £20,000/月计）与既有通风柜不可用需新购（最坏 £150,000）。",
...
hipo9d2:"是否先行冻结 OEB 目标等级与用户需求（BoD）？概念文件未定义等级、隔离器仅按暂列金额计，该口径直接决定设备规格与可选供应商范围。",
hipo9d3:"是否安排 AHU 状况核查与既有通风柜可用性确认？二者为风险登记册中金额最高的两项（最可能 £250,000 / 最坏 £150,000）。",
pDecT:"组合 · 提请关注", pDecS:"",
...
p2r1n:"B902 east extension", p2r1s:"New 4-floor + mezzanine", p2r1o:"£78.1M", p2r1d:"Complete Aug 2030",
...
p2rebase:"Programme basis: the first three lines were estimated from a June 2026 start and are now re-baselined to a September 2026 start (+3 calendar months); scope and cost are unchanged. The HIPO lab concept programme already starts at the 01 Sep 2026 funding approval.",
...
extK3:"Aug 2030", extK3d:"Programme end (+3 months)",
...
extB3:"FS complete; RIBA 2 Concept re-baselined to Oct 2026 (was Jul 2026, +3 months).",
...
extGSub:"Master programme · re-baselined to a Sep 2026 start (+3 months on the June basis)",
...
hipoB1:"Scope: alterations to the existing G-128 suite forming lab and write-up areas, 215 m² GIFA; isolators carried as provisional allowances (4 nr × £150,000 + 1 nr × £100,000).",
...
hipoB5:"To confirm: the concept documents do not define a target OEB level (isolators are provisional allowances); the client equipment figure refers to a separate equipment list not included in this pack.",
...
hipoS2:"Isolators & lab equipment", hipoS2a:"Isolator provisional allowances: 4 nr × £150,000 + 1 nr × £100,000 (£700,000 total)",
hipoS2b:"Fume cupboards: existing assumed serviceable, only a £20,000 servicing/repair allowance; new units remain a TBC item on the programme",
hipoS2c:"SF6 testing allowance £20,000; furniture allowance covers lockers, step-over bench, mobile benches, BIBO bins",
...
hipo9d2:"Freeze the target OEB level and user requirements (BoD) first? The concept documents define no level and isolators are provisional allowances, so this directly sets equipment specification and the available supplier pool.",
hipo9d3:"Commission the AHU condition survey and confirm whether the existing fume cupboards can be re-used? These are the two largest register items (£250,000 most likely / £150,000 worst case).",
...
<section class="slide">${ganttHTML(GANTT_EXT,new Date("2026-07-01"),new Date("2030-08-07"),EXT_KEYS,["2026","2027","2028","2029","2030"],"''' + TODAY + r'''","extGSub",{d:"extLegD",p:"extLegP",b:"extLegB",c:"extLegM",today:"extToday"})}</section>
...
<div class="card"><h3>${t("hipoS2")}</h3><ul style="font-size:.78rem;margin-top:.35rem"><li>${t("hipoS2a")}</li><li>${t("hipoS2b")}</li><li>${t("hipoS2c")}</li></ul></div>
