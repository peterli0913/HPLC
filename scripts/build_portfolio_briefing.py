...
ext6kOom:"OOM 总价", ext6kBase:"基础建造成本", ext6kRisk:"风险与预备费",
extChL:"项目总投资构成", extChR:"基础建造费分项",
...
pEnd:"谢谢", pEnds:"",
extCost:{wsum:"工程费小计",pre:"临建 (8%)",ohp:"管理费利润 (5%)",base:"基础建造成本",oth:"其他项目费",od:"设计费 (8%)",ob:"BREEAM",os:"勘测",op:"规划",rr:"风险登记册",cont:"风险与预备费合计",c15:"设计发展 (15%)",c25:"施工设备 (25%)",c10:"业主 (10%)"},
...
ext6kOom:"Total OOM", ext6kBase:"Base construction", ext6kRisk:"Risk & contingency",
extChL:"Total OOM composition", extChR:"Base construction split",
...
extCost:{wsum:"Works subtotal",pre:"Prelims (8%)",ohp:"OH&P (5%)",base:"Base construction",oth:"Other project costs",od:"Design (8%)",ob:"BREEAM",os:"Surveys",op:"Planning",rr:"Risk register",cont:"Risk & contingency",c15:"Design dev. (15%)",c25:"Constr./equip. (25%)",c10:"Client (10%)"},
...
const L=I18N[lang].extCost;
return `<div class="cost-total-bar"><span>${t("ext5oom")}</span><span>${fm(EXT_OOM)}</span></div>
...
<div class="cost-leaf"><span>0 Facilitating</span><span>${fm(254960)}</span></div>
<div class="cost-leaf"><span>1–4 Works packages</span><span>${fm(15726088)}</span></div>
<div class="cost-leaf"><span>5 Building services (incl. process)</span><span>${fm(9532716)}</span></div>
<div class="cost-leaf"><span>7–8 External / existing</span><span>${fm(2063520)}</span></div>
</div></details>
<div class="cost-leaf" style="padding:.3rem .55rem"><span>${L.pre}</span><span>${fm(3357458)}</span></div>
<div class="cost-leaf" style="padding:.3rem .55rem"><span>${L.ohp}</span><span>${fm(2266284)}</span></div>
<details class="cost-item" open><summary><span class="cost-label">${L.base}</span><span class="cost-amt">${fm(EXT_BASE)}</span></summary></details>
...
<div class="cost-leaf" style="padding:.3rem .55rem"><span>${L.rr}</span><span>${fm(626600)}</span></div>
<details class="cost-item"><summary><span class="cost-label">${L.cont}</span><span class="cost-amt">${fm(EXT_RISK)}</span></summary>
<div class="cost-children">
...
<div class="invest-kpi"><div class="ik-val">${fm(EXT_BASE)}</div><div class="ik-lbl">${t("ext6kBase")}</div></div>
<div class="invest-kpi"><div class="ik-val">${fm(EXT_RISK)}</div><div class="ik-lbl">${t("ext6kRisk")}</div></div></div>
<div class="grid-2">
...
{label:zh?"其他":"Other",data:[4062357],backgroundColor:"#009688"},
{label:zh?"风险预备":"Risk",data:[EXT_RISK],backgroundColor:"#c9a227"}]},
options:{indexAxis:"y",responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom"}},
scales:{x:{stacked:true,max:EXT_OOM*1.02,ticks:{callback:v=>"£"+(v/1e6).toFixed(1)+"M"}},y:{stacked:true,display:false}}}});
new Chart(document.getElementById("cExt2"),{type:"doughnut",data:{labels:[zh?"土建外围":"Civils",zh?"建筑机电":"M&E",zh?"工艺设备":"Process",zh?"临建":"Prelims",zh?"OH&P":"OH&P"],
datasets:[{data:[9186428,9532716,23249083,3357458,2266284],backgroundColor:["#1a4a6e","#2e6da4","#5a8ab8","#009688","#4db6ac"]}]},
options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"right",labels:{font:{size:9}}}}}});
chartsBuilt.ext=true;
}
