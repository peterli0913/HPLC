...
s3d1:"可行性报告及附录（费用、风险、进度、设备清单）",s3d2:"BREEAM 预评估约 62.93%",
...
s8out1:"业主自购工艺设备（费用计划 excluded）",s8out2:"通胀、汇率、税费",
...
ganttSub:"时间轴（条块长度按日历比例；悬停查看说明）",
gFs:"可行性 RIBA 1",gR2:"概念 RIBA 2",gR3:"方案 RIBA 3",gPlan:"规划审批",
gR4:"详细 RIBA 4",gPre:"施工准备",gR5:"施工 RIBA 5",gComm:"调试",gEnd:"验证/竣工",
...
s10r4p:"RIBA 4 详细",s10r4t:"2027-10 — 2028-06",s10r4n:"长周期采购 2028-03 起",
...
pre:"预备费 (8%)",ohp:"承包商管理费与利润 (5%)",base:"基础建造成本",
...
s3d1:"Feasibility report & appendices (cost, risk, programme, equipment)",s3d2:"BREEAM pre-assessment ~62.93%",
...
s8out1:"Client-purchased process equipment (excluded per cost plan)",s8out2:"Inflation, FX, tax",
...
ganttSub:"Timeline (bar length ∝ calendar duration; hover for detail)",
gFs:"Feasibility RIBA 1",gR2:"Concept RIBA 2",gR3:"Scheme RIBA 3",gPlan:"Planning",
gR4:"Detailed RIBA 4",gPre:"Pre-construction",gR5:"Construction RIBA 5",gComm:"Commissioning",gEnd:"Validation / complete",
...
s10r4p:"RIBA 4 Detailed",s10r4t:"Oct 2027 – Jun 2028",s10r4n:"Long-lead from Mar 2028",
...
pre:"Preliminaries (8%)",ohp:"Contractor OH&P (5%)",base:"Base construction",
...
const keys=["gFs","gR2","gR3","gPlan","gR4","gPre","gR5","gComm","gEnd"];
let rows="";
...
<div class="grid-2"><div class="card"><div class="chart-wrap"><canvas id="c1"></canvas></div></div>
<div class="card"><div class="chart-wrap"><canvas id="c2"></canvas></div></div></div></section>
...
<ul style="font-size:.82rem;margin-left:1rem"><li>${t("s8out1")}</li><li>${t("s8out2")}</li><li>${t("s8out3")}</li><li>${t("s8out4")}</li></ul></div></div>
<p style="font-size:.78rem;color:var(--muted);margin-top:.5rem">${t("s8disc")}</p></section>

<section class="slide"><h1>${t("s9t")}</h1><h2>${t("s9s")}</h2>${ganttHTML()}</section>
...
const zh=lang==="zh";
new Chart(document.getElementById("c1"),{type:"bar",data:{labels:[zh?"OOM":"OOM"],datasets:[
{label:zh?"直接工程费":"Works",data:[41968226],backgroundColor:"#0f2b46"},
{label:zh?"预备费与OH&P":"Prelims/OH&P",data:[5623742],backgroundColor:"#1a4a6e"},
{label:zh?"其他项目费":"Other",data:[4062357],backgroundColor:"#009688"},
{label:zh?"风险与预备费":"Risk/cont.",data:[26453763],backgroundColor:"#c9a227"}]},
options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom"}},
scales:{x:{stacked:true},y:{stacked:true,ticks:{callback:v=>"£"+(v/1e6).toFixed(1)+"M"}}}}});
new Chart(document.getElementById("c2"),{type:"doughnut",data:{labels:[zh?"工艺设备":"Process",zh?"机电":"MEP",zh?"外场":"External",zh?"上部":"Super",zh?"基础":"Sub",zh?"其他":"Other"],
datasets:[{data:[23249083,9532716,1859520,3942636,1499680,2720049],backgroundColor:["#0f2b46","#1a4a6e","#2e6da4","#009688","#4db6ac","#90a4ae"]}]},
